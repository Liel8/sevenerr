...
// import { useSelector } from 'react-redux'
// import { useNavigate } from 'react-router-dom'
// import { loadSellerOrders, updateOrder } from '../store/actions/orders.actions'
// import { loadGigs } from '../store/actions/gig.actions'
// import { OrdersTable } from '../cmps/OrdersTable'
...
//   const user   = useSelector(state => state.userModule.user)
//   // עתה ה־state של orders מכיל רק הזמנות שבהן המשתמש הוא SELLER
//   const orders = useSelector(state => state.orderModule.orders)
//   const gigs   = useSelector(state => state.gigModule.gigs)
...
//   // כל הגיגים שהמשתמש הוא OWNER שלהם
//   const userGigs = gigs.filter(gig => gig.owner?._id === user._id)
//   const isExpanded = visibleCount >= userGigs.length
//   const visibleGigs = userGigs.slice(0, visibleCount)

//   // since we already loaded only SELLER orders, אין צורך בסינון נוסף
//   const asSellerOrders = orders
...
//                 <ul>
//                   {user.lang?.length
//                     ? user.lang.map((lng, idx) => (
//                         <li key={idx}>
//                           <span className="title">{lng}</span> –{' '}
//                           <span className="sub-title">Fluent</span>
//                         </li>
//                       ))
...
export function UserProfile() {
  ...
  const user   = useSelector(state => state.userModule.user)
  // orders עכשיו מורידים רק את ההזמנות שבהן המשתמש הוא SELLER
  const orders = useSelector(state => state.orderModule.orders)
  const gigs   = useSelector(state => state.gigModule.gigs)
  ...
  // ה־state כבר מכיל רק הזמנות SELLER
  const asSellerOrders = orders
    ...
    <section className="details-container">
      ...
      <section className="orders-column">
        ...
        <section className="my-gigs">
          ...
          <div className="gigs-list" ref={gigsListRef}>
            {visibleGigs.map(gig => (
              <div key={gig._id} className="my-gig-item">
                <button
                  className="my-gig-btn"
                  onClick={() =>
                    navigate(`/gig/${gig._id}`, { state: { gig } })
